...
export TARGET_HASH=""
export OUTPUT_DIR=""
export LOG_FILE="/tmp/ad_audit_bootstrap.log"   # log temporal hasta configurar OUTPUT_DIR
export REPORT_FILE=""
export START_TIME; START_TIME=$(date +%s)
export WORDLIST_PASS="/usr/share/wordlists/rockyou.txt"
export WORDLIST_USERS="/usr/share/seclists/Usernames/xato-net-10-million-usernames-dup.txt"

# ─── Herramientas requeridas y sus paquetes/instrucciones ────────────────────
declare -A TOOL_INSTALL
TOOL_INSTALL=(
    [rpcclient]="samba-common-bin (apt install samba-common-bin)"
    [smbclient]="smbclient (apt install smbclient)"
    [smbmap]="smbmap (pip install smbmap  o  apt install smbmap)"
    [netexec]="netexec (pip install netexec)"
    [ldapsearch]="ldap-utils (apt install ldap-utils)"
    ...
    [bloodhound-python]="bloodhound (pip install bloodhound)"
    [neo4j]="neo4j (apt install neo4j)"
    [impacket-GetNPUsers]="impacket (pip install impacket)"
    [impacket-GetUserSPNs]="impacket (pip install impacket)"
    [impacket-secretsdump]="impacket (pip install impacket)"
)
...
check_tool() {
    ...
    if [[ -n "${TOOL_INSTALL[$tool]:-}" ]]; then
        echo -e "    ${DIM}→ Instala con: ${TOOL_INSTALL[$tool]}${RESET}"
    fi
    return 1
}
